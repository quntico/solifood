...
const AdminModal = ({ isOpen, onClose, themes = {}, setThemes, activeTheme, setActiveTheme, onCloneClick, onPreviewUpdate }) => {
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          if (error.code === '42703' || error.code === 'PGRST204' || error.message.includes("Could not find the") || error.message.includes("column")) {
            console.warn("[AdminModal] Column missing in DB, removing from payload and retrying...", error.message);
            // Extract column name from error message (PostgreSQL or PostgREST format)
            ...
            if (columnName && attemptData.hasOwnProperty(columnName)) {
              ...
              continue; // Retry after manual cleanup
            }
          } else {
            lastError = error;
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[300] p-4" onClick={onClose}>
